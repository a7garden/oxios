...
- **RFC-018 b.1 — chunking / normalizer / hyperbolic moved to `oxios-memory`** — Three math/text-utility modules extracted from `oxios-kernel::memory` to the new `oxios-memory` crate. `oxios-kernel` now depends on `oxios-memory` (one-way, no cycle). All public items are re-exported from `oxios-kernel` for back-compat. The pre-existing `HyperbolicEmbedding::{build_from_sqlite,persist_to_sqlite,restore_from_sqlite}` methods were removed (they referenced kernel-only `SqliteMemoryStore` and would have created a cycle); functionality returns in b.8. See `docs/rfc-018-memory-extraction-strategy.md` for the full plan (9 sub-phases).
- **RFC-018 b.2 — embedding providers (TfIdf, GGUF) moved to `oxios-memory`** — The `embedding` module and its `gguf` sub-module moved from `oxios-kernel` to `oxios-memory`. The `embedding-gguf` feature flag moved with them (now declared on `oxios-memory`); the kernel re-declares it as a passthrough so existing `oxios-kernel/embedding-gguf` gating still works. The pre-existing `TextVector` type also moved to `oxios-memory` (it was a hidden coupling — the `TfIdfEmbeddingProvider` depends on it). `llama-gguf` and `hf-hub` deps moved from kernel to memory. The root `oxios` binary now depends on `oxios-memory` directly so the feature chain works (`oxios` → `oxios-memory/embedding-gguf`).
- **RFC-018 b.3 — core types + root_index + quota moved to `oxios-memory`** — Core data types (`MemoryType`, `MemoryTier`, `ProtectionLevel`, `MemoryEntry`) and the `root_index` and `quota` modules moved to `oxios-memory`. The types are a hidden coupling — `decay`, `auto_classify`, `auto_protect`, `compaction`, `flash_attention`, and `graph` (planned for b.4-b.5) all depend on them. Extracting them now lets the next phases proceed without a cycle. `MemoryManager` (the central runtime type that uses these) remains in `oxios-kernel` until b.7. All types are re-exported from `oxios-kernel` for back-compat — `use oxios_kernel::MemoryType;` continues to work. `chrono` dep added to `oxios-memory` for `MemoryEntry` timestamps.
