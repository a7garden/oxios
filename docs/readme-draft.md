...
```bash
oxios chat "Explain this codebase"        # Interactive chat
...
```bash
oxios program install ./my-program     # Install from directory
oxios program list                      # List installed programs
oxios program enable code-review        # Enable a program
oxios program disable debug             # Disable a program
```
...
```bash
oxios cron list                           # List scheduled jobs
oxios cron create --schedule "0 9 * *"    # Daily at 9 AM
oxios cron trigger <id>                    # Run on demand
```
...
oxios log                View logs
oxios program            Program management (install, list, enable, disable)
oxios models             List available LLM models
...
Full REST API on **port 4200** with 50+ endpoints. Auth middleware on all `/api/*` routes.
