...
describe('adaptChunk — reasoning delta threading', () => {
  it('forwards `source` on a no-subtype reasoning delta', () => {
    ...
    expect(result.events).toEqual([
      { kind: 'reasoning.delta', messageId: 'm1', text: 'compaction complete', source: 'compaction' },
    ])
  })
  ...
  it('omits `source` for reasoning deltas without one', () => {
    ...
    expect(result.events).toEqual([
      { kind: 'reasoning.delta', messageId: 'm1', text: 'thinking…' },
    ])
  })
  ...
  it('does not emit a source on reasoning.start / reasoning.end', () => {
    ...
    const start = adaptChunk(
      { type: 'reasoning', subtype: 'start' },
      { msgId: 'm1' },
    )
    const end = adaptChunk(
      { type: 'reasoning', subtype: 'end' },
      { msgId: 'm1' },
    )
